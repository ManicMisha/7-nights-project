// Deterministic noise utilities: seeded PRNG, 2D/3D simplex noise, fBm and
// integer hashes. Everything here is pure and reproducible from a seed, so
// the same seed always yields the same world.

/** mulberry32 — tiny, fast, good-quality 32-bit seeded PRNG. */
...
/** Converts any string (or number) into a 32-bit seed. */
...
/** Integer hash of a 3D lattice point → [0, 1). */
...
export class SimplexNoise {
  ...
  /** Fractal Brownian motion over 2D simplex noise, normalised to ~[-1, 1]. */
  fbm2D(x, y, octaves, lacunarity = 2, gain = 0.5) {
    let amp = 1;
    let freq = 1;
    let sum = 0;
    let norm = 0;
  ...
  /** Ridged multifractal noise in [0, 1] — sharp crests for mountain ranges. */
  ridged2D(x, y, octaves) {
    let amp = 0.5;
    let freq = 1;
    let sum = 0;
    let norm = 0;
