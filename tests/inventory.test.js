...
test('adding items fills existing stacks before empty slots', () => {
  ...
  inv.slots[3] = { id: BLOCK.DIRT, count: 60 };
  assert.equal(inv.add(BLOCK.DIRT, 10), 0);
  assert.deepEqual(inv.slots[3], { id: BLOCK.DIRT, count: MAX_STACK });
  assert.deepEqual(inv.slots[0], { id: BLOCK.DIRT, count: 6 });
});

test('a full inventory returns the leftover count', () => {
  const inv = new Inventory();
  for (let i = 0; i < INVENTORY_SIZE; i++) inv.slots[i] = { id: BLOCK.STONE, count: MAX_STACK };
  assert.equal(inv.add(BLOCK.SAND, 5), 5);
});

test('consuming the last item empties the slot', () => {
  const inv = new Inventory();
  inv.slots[0] = { id: BLOCK.TORCH, count: 1 };
  inv.consumeSelected();
...
test('clicking slots picks up, swaps, merges and splits stacks', () => {
  const inv = new Inventory();
  inv.slots[0] = { id: BLOCK.LOG, count: 10 };
  inv.slots[1] = { id: BLOCK.LOG, count: 5 };
  inv.slots[2] = { id: BLOCK.SAND, count: 3 };

  let cursor = inv.clickSlot(0, null); // pick up 10 logs
  assert.deepEqual(cursor, { id: BLOCK.LOG, count: 10 });
  ...
  assert.deepEqual(cursor, { id: BLOCK.SAND, count: 3 });
  assert.deepEqual(inv.slots[2], { id: BLOCK.LOG, count: 8 });
});
